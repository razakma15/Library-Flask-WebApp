...
def create():
    ...
    lst = ["admin","password2","gg"]
    db.execute("INSERT INTO login (username_col,password_col) VALUES (:u,:p)",{"u":lst[2],"p":lst[1]})
    db.commit()
...
@app.route("/csv")
def test_func():
    with open('books.csv', 'r') as read_func:
        reader = csv.reader(read_func)
        book_list = list(reader)
        for x in range(1,5001):
            db.execute("INSERT INTO books (isbn,title,author,year) VALUES (:0,:1,:2,:3)",{"0":book_list[x][0],"1":book_list[x][1],"2":book_list[x][2],"3":book_list[x][3]})
            db.commit()
        return render_template("login.html", name =book_list)
        f.close()"""
